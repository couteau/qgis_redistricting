...
class RdsConfigOptionsPage(QgsOptionsPageWidget):
    ...
    def __init__(self, parent):
        ...
        self.cbEnableCutEdges.setToolTip(
            tr("Calculate cut edges plan-wide compactness score for primary geographic units. Requires installation of Gerrychain addon."))
        ...
        label = QLabel(tr("Redistricting Plugin can benefit from the use of python packages "
        ...
        self.btnInstallPyarrow.setEnabled(not self.canInstallPyogrio() and self.canInstallPyarrow())
        if self.btnInstallPyarrow.isEnabled():
            ...
            if self.canInstallPyarrow():
                self.btnInstallPyarrow.setToolTip(tr("payarrow package is already installed"))
            else:
                self.btnInstallPyarrow.setToolTip(tr("Please install the pyogrio package before installing pyarrow"))
