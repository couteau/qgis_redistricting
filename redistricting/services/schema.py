...
statsSchema1_0_0 = TypedDict(
    "statsSchema1_0_1", {"cut-edges": dict[int, int], "splits": dict[str, list[splitSchema1_0_0]]}
)
...
dataFieldSchema = TypedDict(
    "dataFieldSchema", {"layer": str, "field": str, "caption": str, "sum-field": bool, "pct-base": Optional[str]}
)
dataFieldSchema = TypedDict(
    "dataFieldSchema", {"layer": str, "field": str, "caption": str, "sum-field": bool, "pct-base": Optional[str]}
)
...
statsSchema = TypedDict("statsSchema", {"dist-layer": str, "dist-field": str, "metrics": dict[str, Any]})
...
def migrateSchema1_0_0_to_1_0_1(data: planSchema1_0_0):
    ...
    if vap:
        migrated["pop-fields"].append(
            fieldSchema(layer=migrated["pop-layer"], field=vap, expression=False, caption=tr("VAP"))
        )
    ...
    if cvap:
        migrated["pop-fields"].append(
            fieldSchema(layer=migrated["pop-layer"], field=cvap, expression=False, caption=tr("CVAP"))
        )
    ...
    if splits:
        for f, s in splits.items():
            split_data[f] = [{f: g[0], "districts": {}, "splits": g[1]} for g in s]
...
def migrateSchema1_0_4_to_1_0_5(data: planSchema1_0_4):
    metrics: dict[str, Any] = {
        camel_to_kebab(n): None
        for n, m in metrics_classes.items()
    ...
    if distLayer is not None:
        ...
        with spatialite_connect(geoPackagePath) as db:
            ...
            (
                mindev,
                maxdev,
                metrics["min-polsbypopper"],
                metrics["max-polsbypopper"],
                metrics["mean-polsbypopper"],
                metrics["min-reock"],
                metrics["max-reock"],
                metrics["mean-reock"],
                metrics["min-convexhull"],
                metrics["max-convexhull"],
                metrics["mean-convexhull"],
            ) = cur.fetchone()
