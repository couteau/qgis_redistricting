...
class MockMessageBar(QObject):
    ...
    @overload
    def pushMessage(self, title: str, text: str, level: int = Qgis.MessageLevel.Info, duration: int = -1) -> None: ...
...
class MockQgisInterface(QgisInterface):
    ...
    def addVectorLayer(self, path: str, base_name: str, provider_key: str) -> QgsVectorLayer:
        """Add a vector layer.
    ...
    def activeLayer(self) -> Optional[QgsMapLayer]:
        ...
        return QgsProject.instance().mapLayer(self._activeLayerId) if self._activeLayerId else None
...
def assign_layer(plan_gpkg_path):
    layer = QgsVectorLayer(f"{plan_gpkg_path}|layername=assignments", "test_assignments", "ogr")
...
def dist_layer(plan_gpkg_path):
    layer = QgsVectorLayer(f"{plan_gpkg_path}|layername=districts", "test_districts", "ogr")
    QgsProject.instance().addMapLayer(layer, False)
    return layer
...
def plan(qgis_parent, block_layer, assign_layer, dist_layer):
    p: RdsPlan = deserialize(
        RdsPlan,
        {
            ...
            "dist-field": "district",
            "pop-field": "pop_total",
            "pop-fields": [{"layer": block_layer.id(), "field": "vap_total", "caption": "VAP"}],
            ...
            "geo-fields": [{"layer": assign_layer.id(), "field": "vtdid", "caption": "VTD"}],
            "metrics": {
                "metrics": {
...
def mock_plan(mocker: MockerFixture) -> RdsPlan:
    ...
    plan = mocker.create_autospec(spec=RdsPlan("mock_plan", 5), spec_set=True)
