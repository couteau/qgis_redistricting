...
from qgis.gui import QgsFieldComboBox, QgsMapCanvas
from qgis.PyQt.QtWidgets import QAction, QCheckBox, QDialog, QDialogButtonBox, QLineEdit, QPlainTextEdit, QSpinBox

from redistricting.controllers import EditAssignmentsController
from redistricting.gui import DlgCopyPlan, DlgNewDistrict, DockRedistrictingToolbox, painttool
from redistricting.models import RdsDistrict
from redistricting.services import AssignmentsService, PlanAssignmentEditor, PlanCopier
...
class TestEditAssignmentsController:
    ...
    def mock_dockwidget(self, mocker: MockerFixture):
        widget = mocker.patch("redistricting.controllers.edit.DockRedistrictingToolbox", spec=DockRedistrictingToolbox)
        return widget
    ...
    def controller(  # noqa: PLR0913
        ...
        patch_districttools_dockwidget,
    ):
        controller = EditAssignmentsController(
            qgis_iface, mock_project, mock_planmanager, mock_toolbar, mock_assignments_service
        )
        controller.load()
        return controller
    ...
    def test_create_controller(  # noqa: PLR0913
        self, qgis_iface, mock_planmanager, mock_project, mock_toolbar, mock_assignments_service, mocker: MockerFixture
    ):
    ...
    def test_activeplanchanged_editablelayer_enables_commit(
        self, controller: EditAssignmentsController, mock_plan, mocker: MockerFixture
    ):
    ...
    def test_target_district_changed(
        self, controller_with_plan: EditAssignmentsController, mock_plan, mock_district, mocker: MockerFixture
    ):
        mocker.patch.object(controller_with_plan, "dockwidget", autospec=controller_with_plan.dockwidget)
        controller_with_plan.activePlanChanged(mock_plan)
    ...
    def test_source_district_changed(
        self, controller_with_plan: EditAssignmentsController, mock_plan, mock_district, mocker: MockerFixture
    ):
        mocker.patch.object(controller_with_plan, "dockwidget", autospec=controller_with_plan.dockwidget)
        controller_with_plan.activePlanChanged(mock_plan)
    ...
    def test_target_not_set_cannot_activate(
        self, controller_with_plan: EditAssignmentsController, qgis_canvas: QgsMapCanvas, qtbot: QtBot
    ):
        controller_with_plan.setGeoField("geoid")
        assert controller_with_plan.geoField == "geoid"
        assert controller_with_plan.targetDistrict is None
    ...
    def test_geoid_not_set_geoid_not_set_cannot_activate(
        self, controller_with_plan: EditAssignmentsController, qgis_canvas: QgsMapCanvas, qtbot: QtBot
    ):
    ...
    def test_target_set_geoid_set_can_activate(
        self, controller_with_plan: EditAssignmentsController, qgis_canvas: QgsMapCanvas, qtbot: QtBot
    ):
        district = RdsDistrict(2)
        controller_with_plan.targetDistrict = district
    ...
    def test_activate_with_invisible_layer_sets_error_and_returns_none(
        self,
        controller_with_plan: EditAssignmentsController,
        ...
        assert (
            "Oops!:Cannot paint districts for a plan that is not visible. Please toggle the visibility of plan test's assignment layer."
    ...
    def test_create_district_no_active_plan_logs_error_returns_none(
        self, controller: EditAssignmentsController, mock_newdistrict_dlg, qgis_iface: QgisInterface
    ):
        d = controller.createDistrict()
        assert d is None
        mock_newdistrict_dlg.assert_not_called()
        assert (
            "Oops!:Cannot create district: no active redistricting plan. Try creating a new plan."
            in qgis_iface.messageBar().get_messages(Qgis.MessageLevel.Warning)
        )

    def test_create_district_cancel_returns_none(
        self,
        controller_with_plan: EditAssignmentsController,
        mock_project,
        mock_plan,
    ...
    def test_create_district_complete_plan_returns_none(
        self, controller_with_plan: EditAssignmentsController, mock_project, mock_plan, mock_newdistrict_dlg, qgis_iface
    ):
        mock_newdistrict_dlg.return_value.exec.return_value = QDialog.DialogCode.Accepted
        ...
        mock_project.setDirty.assert_not_called()
        assert "Warning:All districts have already been allocated" in qgis_iface.messageBar().get_messages(
    ...
    def test_edit_district_unassigned_returns(
        self,
        controller_with_plan: EditAssignmentsController,
        mock_project,
        mock_district,
    ...
    def test_edit_district_with_no_argument_and_no_source_target_returns(
        self, controller_with_plan: EditAssignmentsController, mock_project, mock_newdistrict_dlg, mocker: MockerFixture
    ):
        mocker.patch.object(controller_with_plan, "dockwidget")
        mock_newdistrict_dlg.return_value.exec.return_value = QDialog.DialogCode.Accepted
    ...
    def test_edit_district_target_from_signal(
        self,
        controller_with_plan: EditAssignmentsController,
        mock_project,
        mock_district,
    ...
    def test_edit_district_source_from_signal(
        self,
        controller_with_plan: EditAssignmentsController,
        mock_project,
        mock_district,
    ...
    def test_save_as_new_no_active_plan_logs_error_returns(
        self, controller: EditAssignmentsController, mock_copy_dlg, qgis_iface: QgisInterface
    ):
        controller.saveChangesAsNewPlan()
        mock_copy_dlg.assert_not_called()
        assert (
            "Oops!:Cannot save changes to new plan: no active redistricting plan. Try creating a new plan."
            in qgis_iface.messageBar().get_messages(Qgis.MessageLevel.Warning)
        )

    def test_save_as_new_no_pending_changes_logs_error_returns(
        self, controller_with_plan: EditAssignmentsController, mock_copy_dlg, mock_plan, qgis_iface: QgisInterface
    ):
        mock_plan.assignLayer.isEditable.return_value = False
        controller_with_plan.saveChangesAsNewPlan()
        mock_copy_dlg.assert_not_called()
        assert (
            "Oops!:Cannot save changes to no plan: active plan has no unsaved changes."
            in qgis_iface.messageBar().get_messages(Qgis.MessageLevel.Warning)
        )
    ...
    def test_save_as_new_cancel_returns(
        self,
        controller_with_plan: EditAssignmentsController,
        mock_copy_dlg,
        mock_plan,
    ...
    def test_paint_features(
        self,
        controller_with_plan: EditAssignmentsController,
        mock_plan,
        mock_assignments_editor: PlanAssignmentEditor,
        mock_assignments_service: AssignmentsService,
        mocker: MockerFixture,
    ):
    ...
    def test_paint_features_end_edit(
        self,
        controller_with_plan: EditAssignmentsController,
        mock_plan,
        mock_assignments_editor: PlanAssignmentEditor,
    ...
    def test_paint_features_with_geo_field(
        self,
        controller_with_plan: EditAssignmentsController,
        mock_assignments_editor: PlanAssignmentEditor,
        mock_assignments_service: AssignmentsService,
        ...
        controller_with_plan.paintFeatures(features, 1, None, False)
        mock_assignments_service.getEditor.assert_called_once()
        mock_assignments_editor.getDistFeatures.assert_called_once_with("vtdid", {"011271001"}, 1, None)
        mock_assignments_editor.assignFeaturesToDistrict.assert_called_once_with(matching_features, 1)

    def test_end_paint_features(
        self,
        controller_with_plan: EditAssignmentsController,
        mock_assignments_editor: PlanAssignmentEditor,
        mock_assignments_service: AssignmentsService,
    ...
    def test_cancel_paint_features(
        self,
        controller_with_plan: EditAssignmentsController,
        mock_assignments_editor: PlanAssignmentEditor,
        mock_assignments_service: AssignmentsService,
