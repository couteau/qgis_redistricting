...
from qgis.PyQt.QtCore import QEvent, QModelIndex, QPoint, Qt
...
class TestDistrictController:
    ...
    def test_active_plan_changed(
        self, controller_with_active_plan: DistrictController, mock_plan, mocker: MockerFixture
    ):
    ...
    def test_zoom_to_district(self, controller_with_active_plan: DistrictController, mocker: MockerFixture):
        canvas = mocker.patch.object(
            controller_with_active_plan,
            "canvas",
    ...
    def test_zoom_to_district_no_active_plan_returns(
        self, controller: DistrictController, active_plan, mocker: MockerFixture
    ):
        canvas = mocker.patch.object(
            controller, "canvas", mocker.create_autospec(spec=controller.canvas, instance=True)
        )
    ...
    def test_zoom_to_district_no_argument_uses_action(
        self, controller_with_active_plan: DistrictController, mocker: MockerFixture
    ):
    ...
    def test_zoom_to_district_invalid_argument_raises_error(
        self, controller_with_active_plan: DistrictController, mocker: MockerFixture
    ):
        canvas = mocker.patch.object(
            controller_with_active_plan,
    ...
    def test_context_menu_event(self, controller: DistrictController, mocker: MockerFixture):
        ...
        event = QContextMenuEvent(
            QContextMenuEvent.Reason.Mouse, QPoint(0, 0), QPoint(0, 0), Qt.KeyboardModifier.NoModifier
        )
    ...
    def test_copy_data_to_clipboard_with_selection(
        self, controller_with_active_plan: DistrictController, dockwidget, mocker: MockerFixture
    ):
        clipboard = mocker.patch("redistricting.controllers.district.DistrictClipboardAccess")
        mime = mocker.patch("redistricting.controllers.district.QMimeData")
        set_mime = mocker.patch.object(QgsApplication.instance().clipboard(), "setMimeData")
    ...
    def test_edit_district(  # noqa: PLR0913
        self,
        controller_with_active_plan: DistrictController,
        qgis_iface,
        mock_project,
        mock_planmanager,
        mock_toolbar,
        ...
        editctlr = EditAssignmentsController(  # noqa: F841
