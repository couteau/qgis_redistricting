...
from redistricting.models import DeltaList, DeltaListModel, RdsPlan
from redistricting.services import DeltaUpdateService, PlanManager
...
class TestPendingChangesController:
    ...
    def controller(
        ...
        toolbar: QToolBar,
        delta_update_service: DeltaUpdateService,
    ):
    ...
    def test_addplan_with_delta(
        self, controller: PendingChangesController, planmanager: PlanManager, plan, delta: DeltaList
    ):
        controller.load()
        planmanager.appendPlan(plan, True)
        assert controller.model._delta is delta

    def test_update_delta_updates_model(
        self,
        ...
        assert (
            controller_with_active_plan.model.data(
                controller_with_active_plan.model.createIndex(0, 1), Qt.ItemDataRole.DisplayRole
            )
            == "+600"
        )
